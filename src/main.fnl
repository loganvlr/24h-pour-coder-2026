...
(fn creer-tour [nom-p x-p y-p type-key]
  (let [template (. TOUR-TYPES type-key)]
    ...
    :tirs_ecraseur (fn [self]
     ...
     :afficher (fn [self]
                 ;; 1. Moteur d'animation (ralenti pour l'écraseur)
                 (let [is_anim (>= self.sprite 384)
                       max_etat 4]
                   (when is_anim
                     (set self.timer_anim (+ self.timer_anim 1))
                 ...
                 ;; 2. Calcul du mouvement physique "de haut en bas"
                 (let [is_ecraseur (= self.type :ecraseur)
                       offset_y (if is_ecraseur 
                                    (* (math.sin (/ tick 15)) 6) 
                                    0)
                       
                       ;; 3. Dessiner le sprite
                       is_anim (>= self.sprite 384)
                       sprite_actuel (if is_anim 
                                         (+ self.sprite (* (- self.etat 1) 2)) 
                                         self.sprite)
                       rot (if (= self.type :canon) (% (+ self.direction 3) 4) 0)]
                   
                   ;; Application de l'offset_y
                   (spr sprite_actuel self.x (+ self.y offset_y) 0 1 0 rot 2 2)
                   
                   ;; 4. Le texte du niveau (flotte avec la tour)
